...
namespace Can::input
{
	...
	HRESULT process_input_message(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			set(input_source::mouse, input_code::mouse_position_x, { pos.x, 0.0f, 0.0f });
			set(input_source::mouse, input_code::mouse_position_y, { pos.y, 0.0f, 0.0f });
			set(input_source::mouse, input_code::mouse_position, { pos.x, pos.y, 0.0f });
			break;
		}
		case WM_LBUTTONDOWN:
		case WM_RBUTTONDOWN:
		case WM_MBUTTONDOWN:
		{
			...
			const math::v2 pos{ get_mouse_position(lparam) };
			set(input_source::mouse, code, { pos.x, pos.y, 1.0f });
			break;
		}
		case WM_LBUTTONUP:
		case WM_RBUTTONUP:
		...
		{
			{
				...
				const math::v2 pos{ get_mouse_position(lparam) };
				set(input_source::mouse, code, { pos.x, pos.y, 0.0f });
				break;
			}
			break;
		}
		case WM_MOUSEWHEEL:
		{
			set(input_source::mouse, input_code::mouse_wheel, { (f32)(GET_WHEEL_DELTA_WPARAM(wparam)), 0.0f, 0.0f });
			break;
		}

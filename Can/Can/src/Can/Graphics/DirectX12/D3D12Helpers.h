...
namespace Can::graphics::d3d12::d3dx
{
	...
	public:
		...
		constexpr void add(
			...
			D3D12_RESOURCE_BARRIER& barier{ _barriers[_offset] };
		...
		constexpr void add(
			...
			D3D12_RESOURCE_BARRIER& barier{ _barriers[_offset] };
			barier.Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
			barier.Flags = flags;
			barier.UAV.pResource = resource;
		...
		constexpr void add(
			...
			D3D12_RESOURCE_BARRIER& barier{ _barriers[_offset] };
			barier.Type = D3D12_RESOURCE_BARRIER_TYPE_ALIASING;
			barier.Flags = flags;
			barier.Aliasing.pResourceBefore = resource_before;
			barier.Aliasing.pResourceAfter = resource_after;
	...
	{
		const explicit d3d12_descriptor_range(
	...
	{
		constexpr d3d12_root_signature_desc(
			const d3d12_root_parameter* parameters,
			u32 parameter_count,
			const D3D12_STATIC_SAMPLER_DESC* static_sampler = nullptr,
			u32 sampler_count = 0,
		...
		) :D3D12_ROOT_SIGNATURE_DESC1{ parameter_count, parameters, sampler_count, static_sampler, flags }
		{}

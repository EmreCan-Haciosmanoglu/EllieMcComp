...
namespace Can::graphics::d3d12
{
	...
	void descriptor_heap::process_deferred_free(u32 frame_idx)
	{
		...
		std::vector<u32>& indices{ _deferred_free_indices[frame_idx] };
		if (!indices.empty())
		{
	...
	d3d12_buffer::d3d12_buffer(d3d12_buffer_init_info info, bool is_cpu_accessible)
	{
		...
		_buffer = d3dx::create_buffer(info.data, _size, is_cpu_accessible, info.initial_states, info.flags, info.heap, info.allocation_info.Offset);
	...
	d3d12_texture::d3d12_texture(d3d12_texture_init_info info)
	{
		...
		else if (info.heap && info.desc)
		{
			...
			DXCall(device->CreatePlacedResource(
				...
				info.initial_states, clear_value, IID_PPV_ARGS(&_resource)));
		}
		else if (info.desc)
		{

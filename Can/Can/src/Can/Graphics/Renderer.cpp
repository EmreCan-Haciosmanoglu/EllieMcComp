...
namespace Can::graphics
{
	...
	light::type light::light_type() const
	{
		assert(is_valid());
		type light_type;
		gfx.light.get_parameter(_id, _light_set_key, light_parameter::type, &light_type, sizeof(light_type));
		return light_type;
	}
	...
	id::id_type light::entity_id() const
	{
		...
		gfx.light.get_parameter(_id, _light_set_key, light_parameter::entity_id, &entity_id, sizeof(entity_id));
		return entity_id;
	}
